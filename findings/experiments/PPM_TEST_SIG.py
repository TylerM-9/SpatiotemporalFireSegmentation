"""
Testing script for STCNN Joint Prediction + Segmentation using JointSegDecoderNoPPM

This matches your multi-round training setup (DAVIS pretrain + FIRE finetune) EXCEPT:
- It ONLY loads a checkpoint and evaluates on FIRE test split (like your first tester).

You ONLY need to set:
  model_path = "..._best.pth"  (or _final.pth / _epoch*.pth)

Outputs:
- prints metrics for each threshold
- saves example images (input + pred-frame + GT overlay + pred overlay)
- saves a grid of examples
- writes evaluation_results_tX.txt into save dir
"""
...
from network.joint_pred_seg import (
    ...
    FramePredDecoder,
    SegEncoder,
    JointSegDecoderNoPPM,  # <<< IMPORTANT
)
...
# Load model
# -------------------------
def load_stcnn_noppm_model(model_path, num_frame, device):
    print(f"Loading STCNN NoPPM model from: {model_path}")
...
def save_example_image_with_pred(frames, pred_frame, gt_sample, seg_pred, index,
    ...
    """
    frames: [B,3,H,W] current input frame (from FIRE sample['frame'])
    pred_frame: [B,3,h,w] predicted next frame (tanh range typically)
    gt_sample: [H,W,1] or [H,W] (0/1)
    seg_pred: [C,H,W] probabilities after sigmoid
    """
    # --- input frame ---
    ...
    # your earlier tester did (x+1)/2, but FIRE 'frame' might not be tanh-normalized.
    # We'll min-max normalize for display robustness.
    ...
    # If pred is tanh output [-1,1], this makes it look right; otherwise it still clips ok
    ...
    fig.suptitle(f"STCNN-NoPPM Sample {index} - Threshold: {threshold}", fontsize=14, fontweight="bold")
    plt.tight_layout()
...
def main(frame=4, epochs_tag=0, test_thresholds=None, model_round=0):
    ...
    # Keep results organized similarly to your first tester
    model_dir_name = f"{model_name}_round{model_round}"
    save_model_dir = os.path.join(save_root, model_dir_name)
    os.makedirs(save_model_dir, exist_ok=True)

    model = load_stcnn_noppm_model(model_path, frame, device)
    if model is None:
        return

    print(f"Testing STCNN-NoPPM round={model_round}")
    print(f"Thresholds: {test_thresholds}")

    # FIRE test set (same as your STCNN tester)
    ...
    for threshold in test_thresholds:
        ...
        with torch.no_grad():
            for ii, sample_batched in enumerate(testloader):
                ...
                # multi-scale logits -> use last (finest)
                seg_logits = seg_res[-1] if isinstance(seg_res, list) else seg_res

                seg_prob = torch.sigmoid(seg_logits)
                seg_prob = seg_prob[0].cpu().numpy()  # (C,H,W)
        ...
        print(f"Architecture: STCNN + JointSegDecoderNoPPM")
        ...
        with open(results_file, "w") as f:
            f.write("STCNN-NoPPM Evaluation Results\n")
            f.write("=" * 50 + "\n")
            f.write(f"Checkpoint: {model_path}\n")
            f.write("Architecture: STCNN + JointSegDecoderNoPPM\n")
...
def test_all_rounds(frame=4, epochs_tag=0, test_thresholds=None, num_rounds=3):
    """
    Convenience helper if your checkpoints follow the multi-round naming convention
    from the NoPPM training script I gave you earlier.
    """
    if test_thresholds is None:
        test_thresholds = [0.5]
    ...
    for round_num in range(num_rounds):
        ...
        # Update this pattern IF your folder / filename differs
        model_path = os.path.join(
            save_root,
            f"STCNN_NoPPM_FIRE{frame}_round{round_num}",
            f"STCNN_NoPPM_FIRE{frame}_round{round_num}_best.pth"
        )
...
if __name__ == "__main__":
    ...
    # Option 1: test a single checkpoint
