...
# Import your custom DeepLabV3+ implementation
from network.deeplabs import DeepLabV3Plus, load_pretrained_deeplabv3plus, adapt_num_classes
...
class TrainingConfig:
    def __init__(self):
        ...
        self.model_name = 'DeepLabV3Plus_FineTuned'
        self.validation_freq = 10

        # DeepLabV3+ specific parameters
        self.backbone = "resnet50"  # Options: 'resnet50', 'resnet101', 'mobilenet_v2'
        self.output_stride = 16  # Options: 8 or 16
        self.num_classes = 1  # Binary segmentation
        self.input_size = 256  # Input image size (256x256)
        self.pretrained = True  # Use pretrained weights
        self.pretrained_type = 'COCO_WITH_VOC_LABELS_V1'  # Use COCO+VOC pretrained or None
...
class Trainer:
    ...
    def _setup_tensorboard(self) -> SummaryWriter:
        ...
        return SummaryWriter(log_dir=log_dir, comment='-deeplabv3plus')
    ...
    def _initialize_network(self):
        ...
        """
        print(f"Initializing DeepLabV3+ with {self.config.backbone} backbone")
        print(f"Output stride: {self.config.output_stride}")
        print(f"Input size: {self.config.input_size}x{self.config.input_size}")

        if self.config.pretrained and self.config.pretrained_type:
            # Load fully pretrained model (COCO + VOC trained)
            print(f"Loading pretrained weights: {self.config.pretrained_type}")
            net = load_pretrained_deeplabv3plus(
                num_classes=21,  # VOC has 21 classes
                backbone=self.config.backbone,
                weights=self.config.pretrained_type,
                input_size=self.config.input_size
            )

            # Adapt to binary segmentation (1 class)
            if self.config.num_classes != 21:
                print(f"Adapting model from 21 classes to {self.config.num_classes} class(es)")
                net = adapt_num_classes(net, self.config.num_classes)
        else:
            # Create model with pretrained backbone only
            print("Creating model with pretrained backbone only")
            net = DeepLabV3Plus(
    ...
    def _setup_optimizer(self, net):
        """
        Setup optimizer with different learning rates for backbone and decoder
        """
        # Get backbone and decoder parameters
        backbone_params = list(net.get_backbone_params())
        decoder_params = list(net.get_decoder_params())

        # Option 1: Same LR for all parameters
        # optimizer = optim.Adam(net.parameters(), lr=self.config.lr, weight_decay=self.config.wd)

        # Option 2: Different LR for backbone (pre-trained) and decoder
        optimizer = optim.Adam([
            {'params': backbone_params, 'lr': self.config.lr * 0.1},  # Lower LR for pre-trained backbone
            {'params': decoder_params, 'lr': self.config.lr}  # Higher LR for decoder
        ], weight_decay=self.config.wd)

        print(f"Optimizer setup - Backbone LR: {self.config.lr * 0.1:.6f}, Decoder LR: {self.config.lr:.6f}")
    ...
    def train(self):
        ...
        # Optional: Freeze BatchNorm for fine-tuning with small batches
        # Uncomment if you're experiencing BatchNorm issues
        # net.freeze_bn()
...
def main():
    ...
    print(f"Output Stride: {config.output_stride}")
