...
BACKBONE = "resnet50"  # or "resnet101", "mobilenet_v2"
NUM_CLASSES = 1  # Binary segmentation
OUTPUT_STRIDE = 16
INPUT_SIZE = 256  # Will auto-adjust ASPP rates to (3, 6, 9)


def load_deeplabv3plus_model(model_path, device):
    """
    Load DeepLabV3+ model from checkpoint
    """
    print(f"Loading DeepLabV3+ model from: {model_path}")

    # Import the model (assumes deeplabv3plus.py is in the same directory or PYTHONPATH)
    from network.deeplabs import DeepLabV3Plus

    # Initialize DeepLabV3+ architecture (same as training)
    model = DeepLabV3Plus(
...
def main(epochs, test_thresholds=None):
    """
    Main testing function for DeepLabV3+ segmentation model
    ...
    model = load_deeplabv3plus_model(model_path, device)
    if model is None:
        return
    ...
    for threshold in test_thresholds:
        ...
        # Freeze BatchNorm for stable inference (important for DeepLabV3+)
        model.freeze_bn()
        ...
        with torch.no_grad():
            for ii, sample_batched in enumerate(testloader):
                ...
                # Convert output to probability (apply sigmoid for binary segmentation)
        ...
        if processed_count > 0:
            ...
            with open(results_file, 'w') as f:
                f.write("DeepLabV3+ Segmentation Model Evaluation Results\n")
                f.write("=" * 50 + "\n")
                f.write(f"Model: {model_name}\n")
                f.write(f"Backbone: {BACKBONE}\n")
                f.write(f"Output Stride: {OUTPUT_STRIDE}\n")
...
if __name__ == "__main__":
    ...
    main(epochs=201, test_thresholds=test_thresholds)