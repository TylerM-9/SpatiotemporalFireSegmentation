...
def initialize_netG(net, input_frame_nums=4):
    ...
    resnet = models.resnet101(pretrained=True)  # For newer versions, use weights=ResNet101_Weights.IMAGENET1K_V1
    pretrained_dict = resnet.state_dict()
    model_dict = net.state_dict()
    
    # 1. Filter out unnecessary keys (only keep existing layer weights)
    pretrained_dict = {k: v for k, v in pretrained_dict.items() if k in model_dict}
    
    # Handle first convolution layer expansion for multi-frame input
    weight = pretrained_dict['conv1.weight']
    # Replicate weights across input channels for temporal frames
...
def initialize_netD(net):
    print("Loading weights from PyTorch GoogleNet (online)")
    # Load the pretrained Inception v3 model from torchvision
    inception = models.inception_v3(pretrained=True)  # For newer versions, use weights=Inception_V3_Weights.IMAGENET1K_V1
